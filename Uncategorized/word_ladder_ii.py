...
class Solution:
    ...
    def findLadders(self, beginWord: str, endWord: str, wordList: List[str]) -> List[List[str]]:
        ...
        mem = {}
        mem[endWord] = [[endWord]]
        self.bfs(endWord, wordList, mem)
        neighbors = self.get_neighbor(beginWord, wordList)
        all_sol = []
        min_length = None
        for neighbor in neighbors:
            for path in mem[neighbor]:
                if min_length is None or len(path) + 1 < min_length:
                    min_length = len(path) + 1
                all_sol.append([beginWord, *path])
        
        sol = []
        for candidate in all_sol:
    ...
    def bfs(self, endWord, wordList, mem):
        queue = [endWord]

        while len(queue) > 0:
            item = queue.pop(0)
            neighbors = self.get_neighbor(item, wordList)
            cur_path = mem[item]
            cur_path_len = len(mem[item][0])
            # print(len(queue), cur_path_len)
            # print(item, neighbors)
            for neighbor in neighbors:
                # print(neighbor)
                if neighbor not in mem:
                    neighbor_path = [[neighbor, *path] for path in cur_path]
                    # print(neighbor_path)
                    # neighbor_path = []
                    # for path in cur_path:
                    #     neighbor_path.append([neighbor, *path])
                    mem[neighbor] = neighbor_path
                    # print(neighbor_path)

                    queue.append(neighbor)
                else:
                    if cur_path_len + 1 <= len(mem[neighbor][0]):
                        neighbor_path = [[neighbor, *path] for path in cur_path]
